...
function buscarPokemon (deslocamento){
    fetch(`http://pokeapi.co/api/v2/pokemon?offset=${deslocamento}&limit=${limite}`).then((resposta) => resposta.json ())
    .then  ((dados) => {
        exibirPokemon(dados.results);
        alterarBotes(deslocamento > 0, dados.next != null);
    });
}

function alterarBotes(anterior, proximo){
